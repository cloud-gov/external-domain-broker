...
def test_load_albs_on_startup(clean_db):
    ...
    DedicatedALBListener.load_alb_listeners(
        {"arn-1": "org1", "arn-2": "org1", "arn-3": "org2"}
    )
    listeners = DedicatedALBListener.query.all()
...
def test_load_albs_on_startup_doesnt_modify_assigned_instances(clean_db):
    listeners = DedicatedALBListener.query.all()
    assert len(listeners) == 0
    DedicatedALBListener.load_alb_listeners({"arn-1": "org1", "arn-2": "org1"})
    listeners = DedicatedALBListener.query.all()
    assert len(listeners) == 2

    DedicatedALBListener.load_alb_listeners({"arn-1": "org1", "arn-2": "org2"})
    listeners = DedicatedALBListener.query.all()
    assert len(listeners) == 2
